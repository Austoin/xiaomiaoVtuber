...
网关每 30 分钟唤醒一次，并检查工作区中的 `HEARTBEAT.md`（`~/.nanobot/workspace/HEARTBEAT.md`）。如果文件中有任务，agent 会执行它们，并把结果发送到你最近活跃的聊天通道。
...
> **注意：** 网关必须正在运行（`xiaomiao gateway`），并且你至少与 Bot 聊过一次，这样它才知道结果要投递到哪个通道。
