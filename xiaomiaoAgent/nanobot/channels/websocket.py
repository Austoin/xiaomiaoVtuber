...
def _is_xiaomiao_unified_session_key(key: str) -> bool:
    return key == XIAOMIAO_UNIFIED_SESSION_KEY


def _xiaomiao_bridge_events_url() -> str:
    return os.environ.get(XIAOMIAO_BRIDGE_EVENTS_URL_ENV) or DEFAULT_XIAOMIAO_BRIDGE_EVENTS_URL


def _build_xiaomiao_bridge_payload(*, role: str, content: str) -> dict[str, str]:
    return {
        "source": XIAOMIAO_BRIDGE_EVENT_SOURCE,
        "channel": XIAOMIAO_BRIDGE_EVENT_SOURCE,
        "chat_id": XIAOMIAO_UNIFIED_CHAT_ID,
        "role": role,
        "content": content,
    }


async def _post_xiaomiao_bridge_event(payload: dict[str, str]) -> None:
    await asyncio.to_thread(_post_xiaomiao_bridge_event_sync, payload)


def _post_xiaomiao_bridge_event_sync(payload: dict[str, str]) -> None:
    data = json.dumps(payload, ensure_ascii=False).encode("utf-8")
    req = request.Request(
        _xiaomiao_bridge_events_url(),
        data=data,
        headers={"Content-Type": "application/json"},
        method="POST",
    )
    with request.urlopen(req, timeout=XIAOMIAO_BRIDGE_TIMEOUT_SECONDS) as response:
        response.read()
...
class WebSocketChannel(BaseChannel):
    ...
    def __init__(
        ...
        *,
        ...
        self._xiaomiao_reply_buffers: dict[str, list[str]] = {}
    ...
    def _handle_sessions_list(self, request: WsRequest) -> Response:
        ...
        # The webui is normally meaningful for websocket-channel chats only.
        # XiaoMiao also has one explicit API-backed shared session that must be
        # visible so Web, QQ, and Agent WebUI can inspect the same conversation.
        cleaned = [
            {k: v for k, v in s.items() if k != "path"}
            for s in sessions
            if isinstance(s.get("key"), str) and self._is_webui_readable_session_key(s["key"])
        ]
    ...
    def _is_webui_readable_session_key(key: str) -> bool:
        return WebSocketChannel._is_webui_session_key(key) or _is_xiaomiao_unified_session_key(key)

    @staticmethod
    def _session_key_for_chat_id(chat_id: str) -> str | None:
        if chat_id == XIAOMIAO_UNIFIED_CHAT_ID:
            return XIAOMIAO_UNIFIED_SESSION_KEY
        return None

    async def _mirror_xiaomiao_bridge_event(
        ...
        if not text:
            return
        await _post_xiaomiao_bridge_event(
            _build_xiaomiao_bridge_payload(role=role, content=text)
        )
    ...
    def _handle_session_messages(self, request: WsRequest, key: str) -> Response:
        ...
        # Keep this read surface aligned with ``/api/sessions`` instead of
        # letting callers probe arbitrary CLI / Slack / Lark history by URL.
        if not self._is_webui_readable_session_key(decoded_key):
    ...
    async def _dispatch_envelope(
        ...
        if t == "message":
            ...
            await self._mirror_xiaomiao_bridge_event(
                chat_id=cid, role="user", content=content,
            )
            await self._handle_message(
                sender_id=client_id,
                chat_id=cid,
                ...
                metadata=metadata,
                session_key=self._session_key_for_chat_id(cid),
            )
            return
    ...
    async def stop(self) -> None:
        ...
        self._xiaomiao_reply_buffers.clear()
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if "kind" not in payload:
            await self._mirror_xiaomiao_bridge_event(
                chat_id=msg.chat_id, role="assistant", content=text,
            )
        raw = json.dumps(payload, ensure_ascii=False)
    ...
    async def send_delta(
        ...
        if meta.get("_stream_end"):
            ...
            reply_text = "".join(self._xiaomiao_reply_buffers.pop(chat_id, []))
            await self._mirror_xiaomiao_bridge_event(
                chat_id=chat_id, role="assistant", content=reply_text,
            )
        else:
            body = {
                "event": "delta",
            ...
            if chat_id == XIAOMIAO_UNIFIED_CHAT_ID and delta:
                self._xiaomiao_reply_buffers.setdefault(chat_id, []).append(delta)
