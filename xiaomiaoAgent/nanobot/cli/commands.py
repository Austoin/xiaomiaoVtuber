...
# Remove default handler and re-add with unified xiaomiaoAgent format
logger.remove()
...
app = typer.Typer(
    ...
    help=f"{__logo__} xiaomiaoAgent - Personal AI Assistant",
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} xiaomiaoAgent[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} xiaomiaoAgent[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} xiaomiaoAgent v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """xiaomiaoAgent - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    """Initialize xiaomiaoAgent configuration and workspace."""
    ...
    if wizard:
        ...
        try:
            ...
            console.print("[yellow]Please run 'xiaomiao onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'xiaomiao agent -m "Hello!"'
    gateway_cmd = "xiaomiao gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} xiaomiaoAgent is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See the chat app docs in this workspace.[/dim]"
    )
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show xiaomiaoAgent runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
...
def gateway(
    ...
    """Start the xiaomiaoAgent gateway."""
    if verbose:
...
def _run_gateway(
    ...
    *,
    ...
    console.print(f"{__logo__} Starting xiaomiaoAgent gateway version {__version__} on port {port}...")
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show xiaomiaoAgent runtime logs during chat"),
):
...
def status():
    ...
    console.print(f"{__logo__} xiaomiaoAgent Status\n")
