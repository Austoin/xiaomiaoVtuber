...
class SessionManager:
    ...
    def _get_session_lock_path(self, key: str) -> Path:
        """Sidecar lock path used to serialize cross-process saves."""
        return self.sessions_dir / f"{self.safe_key(key)}.jsonl.lock"
    ...
    def _merge_messages(
        ...
        merged = list(disk_messages)
        seen = {cls._message_identity(message) for message in merged}
    ...
    @classmethod
    def _merge_disk_messages_into_session(
        cls,
        session: Session,
        ...
        if not disk_session.messages:
            return
        merged = cls._merge_messages(disk_session.messages, session.messages)
    ...
    def _write_session_file(self, session: Session, *, fsync: bool = False) -> None:
        """Write a session JSONL file using the existing atomic replace flow."""
