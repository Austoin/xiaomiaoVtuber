...
@pytest.mark.asyncio
async def test_sessions_routes_expose_xiaomiao_unified_api_session(
    bus: MagicMock, tmp_path: Path
) -> None:
    sm = _seed_many(
        tmp_path,
        [
            "api:xiaomiao-unified",
            "api:other",
            "websocket:alpha",
        ],
    )
    ...
    try:
        ...
        assert listing.status_code == 200
        keys = {s["key"] for s in listing.json()["sessions"]}
        assert keys == {"api:xiaomiao-unified", "websocket:alpha"}

        messages = await _http_get(
            "http://127.0.0.1:29911/api/sessions/api%3Axiaomiao-unified/messages",
            headers=auth,
        )
        assert messages.status_code == 200
        assert messages.json()["key"] == "api:xiaomiao-unified"
    finally:
        await channel.stop()
        await server_task
...
@pytest.mark.asyncio
async def test_xiaomiao_unified_webui_message_uses_api_session_key(
    bus: MagicMock,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_post(_payload: dict[str, Any]) -> None:
        return None

    monkeypatch.setattr(websocket_module, "_post_xiaomiao_bridge_event", fake_post)
    connection = MagicMock()
    connection.remote_address = ("127.0.0.1", 50000)
    channel = _ch(bus)
    ...
    assert message.chat_id == "xiaomiao-unified"
    assert message.session_key_override == "api:xiaomiao-unified"
