...
export function renderServerAuthUiHtml(context: ServerAuthUiContext): string {
  ...
  if (!indexHtml.includes('__XIAOMIAO_SERVER_AUTH_CONTEXT__'))
    throw new Error('ui-server-auth index.html is missing __XIAOMIAO_SERVER_AUTH_CONTEXT__ placeholder')

  return indexHtml.replace('__XIAOMIAO_SERVER_AUTH_CONTEXT__', serializeInlineJson(context))
}
