...
const overlayBridgeReplyVisible = computed(() => Boolean(overlayBridgeReplyText.value.trim()))
...
onMounted(async () => {
  ...
  controller.start()
  overlayBridgeTimer = setInterval(() => {
    void pollOverlayBridgeReply()
  }, 1500)
})

onUnmounted(() => {
  controller.stop()
  if (overlayBridgeTimer) {
    clearInterval(overlayBridgeTimer)
    overlayBridgeTimer = null
  }
})
</script>
...
<template>
  <div :class="['desktop-overlay']">
    ...
    <div
      v-if="overlayBridgeReplyVisible"
      :class="['bridge-reply-banner']"
    >
      {{ overlayBridgeReplyText }}
    </div>
