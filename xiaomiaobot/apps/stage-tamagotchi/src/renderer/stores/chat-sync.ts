...
function createUserHistoryItem(content: string): ChatHistoryItem {
  return {
    id: createRequestId(),
    role: 'user',
    content,
...
export const useChatSyncStore = defineStore('stage-tamagotchi:chat-sync', () => {
  ...
  function appendIngestErrorMessage(payload: IngestCommandPayload, message: string) {
    ...
    const shouldPersistUserTurn = getRetryText(lastMessage) !== payload.text
    const nextMessages = [
      ...currentMessages,
      ...(shouldPersistUserTurn ? [createUserHistoryItem(payload.text)] : []),
      {
        role: 'error',
        content: message,
