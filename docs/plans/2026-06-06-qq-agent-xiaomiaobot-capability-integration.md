...
- `xiaomiaoAgent` 已有记忆层：短期 session、`memory/history.jsonl`、`SOUL.md`、`USER.md`、`memory/MEMORY.md`、Dream 和记忆版本记录。
...
- Computer Use、Twitter、Minecraft 已有 xiaomiaoAgent MCP 安全 profile：默认关闭，启用后只注册显式 `enabled_tools`，低风险/高风险仍由 `ToolRegistry` 和 QQ 确认层二次控制。
- QQ 普通对话已接入 `xiaomiaoAgent API :8900`，但 `qq-group` 当前默认是 `low_risk`，只能用读文件、搜索和 Web 抓取等低风险能力。
- `xiaomiaobot` 还有部分能力尚未从 QQ 侧产品化接入：HomeAssistant、Bilibili、Chess、Claude Code、Browser Extension、stage-pocket 动作化与动态绑定、`memory-pgvector` 合并等。stage-pocket 已完成第一批只读 bridge event 同步。
...
- 工具调用、确认请求、记忆更新和舞台动作都写入统一 bridge event，让 Web/桌面端能看到 QQ 触发的动作结果。
...
- `low` 补充：`markitdown_convert` workspace 文件转 Markdown、`scrapling_get` 公网网页主内容抽取。
...
- API 响应中返回工具事件摘要，供 QQ 和 bridge event 展示。
...
- Computer Use MCP：已完成 opt-in 安全 profile，本机窗口/终端/浏览器动作默认仅白名单 + 确认。
- Minecraft service：已完成 opt-in 安全 profile，查询状态/日志低风险开放，注入聊天/事件/REPL 高风险确认。
- Twitter service：已完成 opt-in 安全 profile，搜索/读取低风险开放，发帖/点赞/转发/登录/保存 session 高风险确认。
- HomeAssistant plugin：读取状态低风险；控制设备高风险确认。

第二批接入：

- Bilibili live chat plugin。
- Chess gamelet。
- Claude Code plugin hook。
- Browser extension 页面上下文。
- stage-pocket 移动端 bridge event 同步第一步已完成；后续补 bridge binding handshake、动态地址配置和专门事件 UI。
...
- API 请求 metadata 正确传入 `AgentLoop.process_direct()`。
- MCP 工具按风险等级过滤，包含 Computer Use、Twitter、Minecraft profile。

前端/Vitest：

- stage bridge event 能渲染 tool start/finish/error。
- confirmation event 不重复追加聊天消息。
- 舞台动作事件不破坏现有聊天同步。
- stage-pocket 能同步 chat/tool/confirmation/memory/stage bridge events 到移动端聊天历史。
...
### Batch 4：xiaomiaobot 第一批能力工具化

- 先做 stage 状态和 TTS/字幕/表情控制。
- 已完成 Computer Use MCP、Minecraft、Twitter 的 Agent MCP 安全 profile。
- HomeAssistant 仍保留为 WIP，不通过 `xiaomiaobot_action` 伪执行。

### Batch 4A：tool 目录低风险工具化

- 已接入 `markitdown_convert`：workspace 内本地文件转 Markdown，拒绝 URI 和 workspace 外路径。
- 已接入 `scrapling_get`：公网 `http/https` GET 主内容抽取，阻断内网/private/link-local/metadata，拒绝浏览器/session/凭据类能力。
- 已把两个工具加入 `low_risk` 可见白名单。
- 后续高风险扩展包括 MarkItDown OCR/云服务、Scrapling fetch/stealth/session/Spider。

### Batch 4B：xiaomiaobot MCP 安全 profile

- 已新增 `tools.computer_use_mcp`：默认关闭，启用后只注册显式工具列表；`low_risk` 模式只开放观察/读取状态，`trusted_confirmed` 才开放终端、桌面点击、键盘、PTY、workflow 等动作。
...
- `xiaomiaobot_action` 仍只执行 stage；MCP profile 服务会提示使用对应 MCP 工具，避免假成功。

### Batch 4C：stage-pocket 只读 bridge event 同步

- 已新增 stage-pocket bridge event 同步模块。
- 已在移动端 `App.vue` 初始化聊天会话后启动 xiaomiao bridge event 轮询，卸载时停止。
...
- 后续仍需 bridge binding handshake、动态 bridge URL、owner 绑定和专门事件 UI。

### Batch 5：事件与前端展示

- 扩展 bridge event schema。
- stage-web/stage-tamagotchi 渲染工具事件和确认事件。
- stage-pocket 同步并展示第一批 bridge events。
- 补 Vitest。
...
### Deep Batch A：舞台动作真实消费

- 为 `stage_action` 定义前端侧最小稳定语义：`say`、`tts`、`subtitle` 触发字幕和语音；其他动作先进入聊天事件流，不静默伪成功。
- 兼容两种事件来源：
  - `xiaomiao_stage`：`result_summary` 是动作名，`content` 是文本。
  - `xiaomiaobot_action`：`content` 是包含 `service/action/payload` 的 JSON。
- 通过事件 ID 做去重，避免轮询重复导致重复播报。
- 保持现有 `/state` 普通聊天轮询逻辑可用。

### Deep Batch B：bridge state 安全过滤

- `/v1/xiaomiao/state` 只反映普通聊天 assistant 回复。
- `confirmation_requested`、`tool_start`、`tool_finish`、`tool_error`、`memory_update`、`stage_action` 不写入 latest desktop state。
- 重新加载持久化事件时同样执行过滤，避免重启后桌面字幕被工具事件污染。

### Deep Batch C：验证矩阵

- Python：新增 bridge state 过滤回归测试。
...
- 高风险动作采用二次确认。
- 不把 xiaomiaobot 前端组件直接暴露给 QQ；统一通过 bridge、server-runtime、MCP 或 Agent tool adapter 调用。
