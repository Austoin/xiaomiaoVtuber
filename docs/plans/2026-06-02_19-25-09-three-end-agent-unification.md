...
# Plan: 三端统一接入 nanobot Agent

## 目标

把 `AuBot stage-web`、`stage-tamagotchi` 桌面端、`xiaomiao` QQ 入口统一到
同一个 nanobot Agent 能力层，并把对话事件同步到可订阅的 bridge 事件流。
...
3. 现有 bridge 只保存每个用户最后一条 assistant 回复：`publish_desktop_state(...)`。
4. `stage-web` 只会发消息到 bridge，不会轮询 QQ/桌面产生的消息。
...
### Batch 2: stage-web 订阅 bridge 事件
...
2. `AuBot/apps/stage-web/src/pages/index.vue`
   - 周期拉取 `/events`，把 QQ/桌面消息合并到当前聊天 session。
3. 必要时只改一个输入组件处理去重元数据。

### Batch 3: 根目录统一 config
...
3. `xiaomiao/unified_config.py` 或现有配置入口
   - 从根目录 `config.json` 读取 nanobot 与 provider 覆盖项。

本批会创建本地 `config.json`，内容只用占位或空值，真实密钥由用户自行填入。

### Batch 4: QQ 使用 nanobot 全工具
...
4. bridge `/events` 验证 web、QQ、desktop 事件同步。
...
1. `desktop_bridge.py` 新增 bridge 事件历史、`/v1/xiaomiao/events`、事件发布与查询 helper。
...
2. `index.vue` 启动 bridge 事件轮询，将 QQ/桌面事件合并进当前 stage-web 聊天 session。
...
3. `main.py` 删除旧 `SearchOnline` 导入，避免普通聊天静默回退到旧 provider。
...
1. `stage-tamagotchi` 主页面新增 bridge `/events` 轮询。
