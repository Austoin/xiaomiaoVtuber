...
脚本会按真实健康状态串行启动。前一步未就绪时，后续终端不会打开；QQ 协议端会复用已登录并监听 `5004` 的现有 NapCat/QQ 进程；其他端口已被旧进程占用时会显示 PID 并停止，不会跳过到后续服务。这样可以保证每个新启动的服务都有自己的独立终端。新启动的终端默认最小化，可从任务栏点开查看日志。脚本也会设置 `NO_PROXY=127.0.0.1,localhost,::1`，避免本机代理影响 `main.py` 直连 NapCat。
