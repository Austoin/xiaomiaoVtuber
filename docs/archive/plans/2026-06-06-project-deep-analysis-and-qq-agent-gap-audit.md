...
- 上一个计划书没有“全面完成”。它的基础设施部分完成度较高，包括 QQ 工具权限、确认码、Agent API `tool_policy`、ToolRegistry 二次拦截、桥接事件元数据、QQ 记忆命令、stage `say/tts/subtitle` 最小闭环。
- 当前增量已经把 Computer Use、Twitter、Minecraft 的 Agent MCP 安全配置档接入配置层：默认关闭，显式启用后使用精确 `enabled_tools`，QQ `low_risk` 仍由 `ToolRegistry.prepare_call()` 二次拦截，高风险工具需要确认后才可见。
- 上一个计划书的最终目标尚未完全达成。尤其是 xiaomiaobot 生态里的 HomeAssistant、Bilibili、Chess、Claude Code、Browser Extension 等能力，仍处于“已有服务/插件/事件面，但尚未形成 QQ 可直接稳定调用的产品级闭环”。stage-pocket 已完成第一批只读桥接事件同步，但还缺桥接绑定握手和动态连接配置。
- 当前项目已经形成三层系统：`xiaomiao` 负责 QQ 与本地桥接服务，`xiaomiaoAgent` 负责 Agent 记忆与工具执行，`xiaomiaobot` 负责舞台、TTS、插件和外部服务生态。
...
定位：QQ 机器人入口、本地桌面桥接、旧命令系统、Agent 权限网关。
...
- 桥接前端连接仍依赖本机固定地址，用户绑定机制还不够通用。

### 2. xiaomiaoAgent

定位：真正的 Agent 大脑，负责记忆、工具调用、MCP、Shell、OpenAI 兼容 API。
...
- `tool/markitdown` 和 `tool/Scrapling` 的第一批低风险能力已接入 Agent 工具层：
  - `markitdown_convert`：Agent 工作区和项目根 `workspace/` 本地文件转 Markdown，支持 QQ 文档落盘后读取。
...
- `Twitter service` 已有 MCP 适配器，包含搜索、时间线、发帖、点赞、转发等。
- `Minecraft service` 有 debug MCP 与运行时上下文。
- `Browser Extension` 有页面上下文桥接。
- `Claude Code plugin` 可把 Claude Code hook 输入转发到通道服务。
...
- stage-pocket 已接入 xiaomiao 桥接事件第一批只读同步，可把聊天/工具/确认/记忆/舞台事件合并进移动端聊天历史。
...
完成得较扎实的是“权限、确认、Agent API、工具策略、桥接事件、QQ 记忆命令、舞台说话/TTS 最小链路”。未全面完成的是“所有 xiaomiaobot 能力都能被 QQ 机器人自然语言或命令直接调用”。
...
| 工具事件返回给 QQ/桥接服务 | 已完成 | `xiaomiao_tool_events` 已进入响应和桥接事件。 |
| QQ 中文记忆命令 | 已完成 | `记忆状态/整理记忆/记忆日志/恢复记忆/新会话/停止任务` 已映射。 |
| `/dream-restore` 高风险确认 | 已完成或基本完成 | 恢复类命令进入高风险确认模型，需要继续保持回归测试。 |
| 桥接事件 schema 扩展 | 已完成 | 已支持 tool/memory/stage/confirmation 元数据。 |
| stage-web/stage-tamagotchi 展示工具事件 | 已完成 | 前端可渲染确认、工具开始/完成/失败、舞台动作等事件。 |
| stage `say/tts/subtitle` 真实消费 | 已完成 | 桌面端可播报和显示字幕。 |
| 桥接状态安全过滤 | 已完成 | 工具事件不再污染最新桌面状态。 |
| 舞台表情/背景/模型切换 | 已完成 | stage-tamagotchi 已消费 `emotion/background/model/status`，失败会显式回传。 |
| MarkItDown 低风险工具 | 已完成 | `markitdown_convert` 已接入 ToolLoader 和 low_risk 白名单，仅允许 Agent 工作区和项目根 `workspace/` 内文件。 |
| Scrapling 低风险工具 | 已完成 | `scrapling_get` 已接入 ToolLoader 和 low_risk 白名单，仅允许公网 GET，阻断内网目标。 |
| Computer Use MCP QQ 接入 | 已完成安全配置档 | 已有显式启用项 `tools.computer_use_mcp`，显式 allowlist 区分 low_risk 和 trusted_confirmed；真实服务仍需用户自行启动。 |
| Minecraft QQ 接入 | 已完成安全配置档 | 已有显式启用项 `tools.minecraft_mcp`，只读状态/日志低风险可见，`execute_repl/inject_*` 需确认。 |
| Twitter QQ 接入 | 已完成安全配置档 | 已有显式启用项 `tools.twitter_mcp`，`search/refresh-timeline/get-my-profile` 低风险可见，发帖/点赞/转发/登录需确认。 |
| HomeAssistant QQ 接入 | 未完成 | 插件入口仍为 `WIP`。 |
| Bilibili QQ 接入 | 未完成 | 插件入口仍为 `WIP`。 |
| Chess QQ 接入 | 未完成 | 缺稳定运行入口和 Agent 工具适配。 |
| Claude Code hook QQ 接入 | 部分完成 | hook 能进通道服务，但不是受控 QQ 工具。 |
| Browser Extension QQ 接入 | 部分完成 | 有桥接/上下文，未接入 Agent 统一工具面。 |
| stage-pocket 事件同步 | 已完成第一步只读同步 | 移动端已轮询 xiaomiao 桥接事件，并把聊天/工具/确认/记忆/舞台事件合并进聊天历史；仍缺桥接绑定握手与动态地址配置。 |
...
`QQ -> xiaomiao/main.py -> agent_backend.py -> xiaomiaoAgent /v1/chat/completions -> AgentLoop -> 桥接事件`
...
`中文别名 -> Agent slash command -> Dream/status/log/restore/new/stop -> 桥接事件`
...
`QQ/Agent -> xiaomiao_stage 或 xiaomiaobot_action -> 桥接 stage_action -> stage-tamagotchi 轮询 -> applyXiaomiaoStageActionEvents -> stage store/character store -> Stage TTS/lipsync/caption/model/background`
...
- `markitdown_convert` 只读 Agent 工作区和项目根 `workspace/` 内本地文件，拒绝 URI 和其它本机路径。
- QQ 群文件上传和普通 file 消息段会进入 `workspace/downloads/qq/`，随后由 `markitdown_convert` 转 Markdown 和总结。
- `scrapling_get` 只读公网 URL，阻断 localhost/private/link-local/云厂商元数据服务，暂不开放浏览器、stealth、会话、cookies/auth/proxy/CDP。

### xiaomiao 桥接事件到 stage-pocket

链路：

`stage-pocket 挂载 -> startXiaomiaoBridgeEventSync -> /v1/xiaomiao/events 轮询 -> 将桥接事件追加到聊天会话`
...
- 只消费并展示事件，不直接执行舞台动作。
- 轮询地址仍使用默认本机桥接 URL，后续需要桥接绑定握手和动态配置。

## 主要缺口

### 缺口 1：舞台动作闭环已补齐，仍需联调打磨
...
- 为更多舞台状态字段补充 UI 可视化，而不是只在桥接事件中返回。

### 缺口 2：服务适配器多为“事件排队”，不是“动作执行”
...
- 建立 `qq-agent-mcp-profile` 安全配置档。
- 低风险只读工具默认可见。
- 中高风险工具必须确认。
- MCP stdio 启动和外部服务写操作必须写入桥接事件。

### 缺口 4：多端桥接同步不完整

当前：

- stage-web 和 stage-tamagotchi 已接桥接事件。
- stage-pocket 已接 xiaomiao 桥接事件第一批只读同步。
...
### 批次 2：Computer Use MCP 安全接入

目标：QQ 白名单可确认后调用本机窗口/终端/浏览器能力。

当前状态：已完成 Agent 配置层安全配置档。该配置档默认不启动服务，启用后只注册显式 `enabled_tools`；QQ `low_risk` 和 `trusted_pending` 下仍只暴露只读工具，`terminal_exec`、桌面点击、键盘、PTY、工作流等动作必须进入 `trusted_confirmed`。
...
- 待真实服务联调时继续确认 MCP 执行结果是否完整写入桥接事件。
...
### 批次 3：Twitter 只读与写操作分层

目标：QQ 可搜索/读取 Twitter，高风险写操作确认。

当前状态：已完成 Agent 配置层安全配置档。`tools.twitter_mcp.enable=true` 后连接本机 Twitter MCP SSE 服务，默认 `enabled_tools` 使用精确列表，不使用 `*`。
...
- 已把 Twitter MCP 适配器接入 Agent MCP 安全配置档。
- 已补 `refresh-timeline`、`get-my-profile` 连字符工具名的低风险过滤测试。
- QQ 简短摘要与前端完整事件展示依赖统一 Agent 工具事件，后续真实服务联调时继续验收。

### 批次 4：Minecraft 状态与注入

目标：先做查询和调试注入，再做真实游戏动作。

当前状态：已完成 Agent 配置层安全配置档。`tools.minecraft_mcp.enable=true` 后连接本机 Minecraft debug MCP，默认使用 streamable HTTP，并通过显式 allowlist 区分状态读取和注入/REPL。
...
- 已把 Minecraft debug MCP 作为只读/高风险分层工具接入 Agent 安全配置档。
- 已补 `get_state/get_logs/get_last_prompt/get_llm_trace` 低风险过滤测试。
- 服务不可用时由 MCP 连接/调用路径显式报错，后续真实服务联调时继续验收桥接 `tool_error` 展示。
...
- 每个插件补 `capability_status`：
  ...
  - `action_ready`
  - `qq_agent_ready`
- `xiaomiaobot_status` 返回真实状态，不只返回桥接在线。
- 已把 stage 标记为 `qq_agent_ready`。
- 已把 Computer Use、Twitter、Minecraft 标记为 `action_ready` + `qq_agent_ready`，并明确它们通过 Agent MCP 安全配置档执行，不通过 `xiaomiaobot_action` 伪造桥接动作。
- HomeAssistant、Bilibili、Chess、Claude Code、Browser Extension 仍按真实情况标记为 `wip`。
- stage-pocket 已完成第一批只读桥接事件同步，但暂不把移动端动作入口标记为可执行能力。
- 文档和工具描述避免把 WIP 能力描述成可执行能力。

### 批次 5A：stage-pocket 桥接事件只读同步

目标：让移动端能看到 QQ/Agent 触发的聊天、工具、确认、记忆和舞台事件。

交付：

- 已新增 stage-pocket 专用桥接事件同步模块。
...
5. Minecraft 状态/注入。
6. WIP 插件状态暴露。
7. stage-pocket 桥接绑定握手和移动端事件 UI 深化。
8. 记忆体系合并评估。
...
- “QQ 直连 Agent 的权限、确认、记忆命令、事件可观测性、舞台 TTS/字幕/表情/背景/模型闭环、stage-pocket 第一批只读桥接事件同步，以及 `tool/` 第一批低风险 MarkItDown/Scrapling 工具”已经完成。
- “QQ 机器人可以直接调用 xiaomiaobot 全部服务/插件能力”没有完成。
- “Computer Use、Minecraft、Twitter 已完成安全配置档，但真实外部服务联调仍待按需启用；HomeAssistant、Bilibili、Chess、Claude Code、Browser Extension、stage-pocket 动作化与绑定配置全部产品化接入”没有完成。
