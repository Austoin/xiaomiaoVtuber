...
- `xiaomiao/desktop_bridge.py:67`：创建本地 HTTP 桥接服务，提供模型、状态和聊天接口。
- `AuBot/apps/stage-tamagotchi/src/renderer/pages/xiaomiao-bridge.ts:52`：读取小喵桥接状态。
- `AuBot/apps/stage-tamagotchi/src/renderer/stores/chat-sync.ts:285`：桌面聊天输入优先走小喵桥接服务。
...
### 阶段 1：让 AuBot stage-web 消费小喵桥接服务

范围控制在最多 3 个文件：

1. 在 `stage-web` 增加小喵桥接客户端或复用 Electron 端桥接逻辑。
2. 在 `stage-web/src/pages/index.vue` 中把语音/文本输入优先转到 `xiaomiao` 桥接服务。
3. 保持桥接不可用时显式失败或走现有提供方路径，具体行为实现前再按当前代码测试决定。

验收标准：

- `stage-web` 能向 `http://127.0.0.1:5519/v1/chat/completions` 发送用户文本。
- 成功后 user/assistant 消息进入当前聊天会话。
- 页面舞台仍能正常渲染，音频/VAD 初始化不被破坏。
- 运行 `pnpm -C AuBot --filter @proj-airi/stage-web typecheck`。

### 阶段 2：让 xiaomiao 桥接服务可选接入 nanobot Agent

范围控制在最多 3 个文件：

1. 在 `xiaomiao` 增加可配置的 Agent 后端。
...
- `xiaomiao` 桥接服务可以调用 `nanobot` API 获得 Agent 回复。
...
2. 启动 `xiaomiao`，确认桥接状态接口 `127.0.0.1:5519/v1/xiaomiao/status`。
3. 启动 `stage-web`，在网页端发送文本或语音。

验收标准：

- `stage-web` 输入文本后，链路经过 `xiaomiao` 桥接服务。
- 配置启用时，`xiaomiao` 桥接回复来自 `nanobot Agent`。
...
1. `stage-web` 桥接接入与类型检查。
2. `xiaomiao` 桥接 Agent 后端与单元测试。
3. 联调脚本或启动验证说明，仅在必要时追加。

## 测试计划

- Python 桥接单测：`test/xiaomiao/test_desktop_bridge.py`。
...
- `xiaomiao` 和 `nanobot` 同时维护上下文时，可能出现会话状态分裂。
- 网络、API Key、模型提供方失败必须显式暴露，不做静默假成功。

## 确认点

已确认进入实现。当前执行小批次：

1. 第一批：计划状态、stage-web 桥接辅助、桌面 ChatArea 文本入口。已完成并通过 typecheck。
2. 第二批：移动端文本入口与页面级录音入口。已完成并通过 typecheck。
3. 第三批：xiaomiao Agent 后端与单元测试。已完成并通过 unittest。
4. 第四批：桌面桥接 HTTP 502 显式错误、QQ 群/私聊自然语言回复统一到 Agent 后端。已完成并通过 unittest 与 py_compile。
...
- 运行时闭环：`xiaomiao 桥接服务 -> nanobot Agent -> 桥接状态` 返回 200，回复内容为 `小喵网页接入测试成功`。
...
- `xiaomiao` 桥接服务 `/v1/xiaomiao/status` 返回 200。
- `xiaomiao` 桥接服务 `/v1/chat/completions` 调用 nanobot Agent 成功。
- `xiaomiao` 桥接服务 `/v1/xiaomiao/state` 写入同一回复状态。
- `stage-web` 浏览器输入链路真实触发 `xiaomiao` 桥接服务，并把 nanobot 回复写入页面聊天历史。
