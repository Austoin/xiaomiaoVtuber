...
describe('useChatSyncStore authority ingest failures', async () => {
   ...
   * })
   */
  it('stores failed user turns before the authority error message', async () => {
    ...
    expect(persistedMessages[1]).toEqual(expect.objectContaining({ role: 'user', content: 'hello' }))
    expect(persistedMessages[2]?.role).toBe('error')
    expect(persistedMessages[2]?.content).toContain('This model is not available in your region')
  ...
  /**
   * @example
   * it('mirrors authority-side ingest failures back into follower history', async () => {
   *   // authority receives follower command failure
   *   // follower snapshot keeps user turn visible and shows the error entry
   * })
   */
  it('broadcasts failed user turns to follower snapshots after authority errors', async () => {
    const authority = useChatSyncStore()
    authority.initialize('authority')

    const peer = new MockBroadcastChannel('airi:stage-tamagotchi:chat-sync')
    const snapshots: Array<{ sessionMessages: Record<string, Array<{ role: string, content: string }>> }> = []
    peer.addEventListener('message', ((event: MessageEvent<any>) => {
      if (event.data?.type === 'session-snapshot') {
        snapshots.push(event.data.snapshot)
      }
    }) as EventListener)

    peer.postMessage({
      type: 'command',
      requestId: 'req-follower',
      senderId: 'peer',
      command: 'ingest',
      payload: {
        text: 'hello follower',
    ...
    await vi.waitFor(() => {
      ...
      expect(latest?.sessionMessages['session-1']).toEqual([
        { role: 'system', content: 'init' },
        expect.objectContaining({ role: 'user', content: 'hello follower' }),
...
/**
 * @example
 * describe('useChatSyncStore xiaomiao bridge integration', () => {
 *   it('prefers the local xiaomiao bridge when available', async () => {})
 * })
 */
describe('useChatSyncStore xiaomiao bridge integration', async () => {
  const { useChatSyncStore } = await import('./chat-sync')
  ...
  beforeEach(() => {
    ...
    mockState = {
      ...
      setSessionMessages: vi.fn((sessionId: string, next: Array<{ role: string, content: string }>) => {
        sessionMessages.value[sessionId] = next
      }),
      getSessionMessages: vi.fn((sessionId: string) => sessionMessages.value[sessionId] ?? []),
   ...
   * @example
   * it('prefers the local xiaomiao bridge when available', async () => {
   *   // authority ingests through local bridge
   *   // session history stores user and assistant turns
   * })
   */
  it('prefers the local xiaomiao bridge when available', async () => {
    const store = useChatSyncStore()
    ...
    expect(mockState.setSessionMessages.mock.calls[0]?.[1]).toEqual([
      expect.objectContaining({ role: 'user', content: '你好' }),
      expect.objectContaining({ role: 'assistant', content: '桥接回复' }),
    ])
