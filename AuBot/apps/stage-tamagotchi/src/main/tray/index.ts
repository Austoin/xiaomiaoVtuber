...
export function buildTrayMenuTemplate(params: TraySetupParams) {
  const { x: areaX, y: areaY, width: areaWidth, height: areaHeight } = screen.getPrimaryDisplay().workArea
  ...
  const captionMenuItems = params.captionWindow
    ? [
        {
          ...
          click: () => {
            void params.captionWindow?.toggleVisibility()
          },
        },
        {
          ...
          submenu: Menu.buildFromTemplate([
            { type: 'checkbox', label: params.i18n.t('tamagotchi.electron.tray.menu.labels.label.follow_window'), checked: params.captionWindow.getIsFollowingWindow(), click: async menuItem => await params.captionWindow?.setFollowWindow(Boolean(menuItem.checked)) },
            {
              ...
              click: async () => {
                await params.captionWindow?.setFollowWindow(false)
                await params.captionWindow?.resetToSide()
              },
            },
          ]),
        },
  ...
  return [
    ...
    ...is.dev || env.MAIN_APP_DEBUG || env.APP_DEBUG
      ? [
          { type: 'header' as const, label: params.i18n.t('tamagotchi.electron.tray.menu.labels.label.devtools') },
...
export function setupTray(params: TraySetupParams): void {
  once(() => {
    ...
    const rebuildContextMenu = debounce((): void => {
      ...
      const contextMenu = Menu.buildFromTemplate(buildTrayMenuTemplate(params))
