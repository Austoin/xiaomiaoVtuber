...
async function playFunction(item: Parameters<Parameters<typeof createPlaybackManager<AudioBuffer>>[0]['play']>[0], signal: AbortSignal): Promise<void> {
  ...
  // NOTICE:
  // Bridge-driven speech (for example QQ/xiaomiao desktop replies) bypasses the normal
  // chat orchestrator hooks that usually call setupAnalyser() + setupLipSync() before
  // opening a speech intent. Without this preparation, audio can still play, but Live2D
  // never receives a lip-sync input node, causing "has sound but no mouth movement".
  // We fix this at the playback boundary so every speech source shares the same mouth-sync
  // initialization path.
  setupAnalyser()
  await setupLipSync()
